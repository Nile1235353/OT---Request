...
<style>
    ...
    .edit-modal-overlay { align-items: flex-start; padding-top: 5vh; justify-content: center; }
    .reject-modal-overlay { align-items: center; justify-content: center; }
    .modal-content {
        background-color: white; padding: 1.5rem 2rem; border-radius: 0.5rem;
        z-index: 1000; width: 90%; box-shadow: 0 10px 25px rgba(0,0,0,0.1);
        margin-bottom: 5vh; animation: fadeIn 0.3s;
    }
    @keyframes fadeIn {
    ...
    .reject-modal-content { max-width: 500px; }
    .new-user-entry {
        display: flex; flex-direction: column; gap: 0.5rem;
        border: 1px solid #e0e0e0; padding: 0.75rem;
        border-radius: 0.375rem; background-color: #f9fafb;
    }
</style>
            ...
            @forelse($pendingRequests as $request)
                <div class="border border-gray-200 p-5 rounded-lg bg-white grid grid-cols-1 md:grid-cols-4 gap-6 items-start shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div class="flex flex-col space-y-3 justify-center h-full pt-2">
                        <button type="button" onclick="openEditModal('editModal-{{ $request->id }}')" 
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            @if($history->status == 'approved')
                                <button type="button" onclick="openRejectModal('{{ $history->id }}')" 
        ...
        <div class="modal-content edit-modal-content" onclick="event.stopPropagation();">
            ...
            <form action="{{ route('approvals.approve', $request->id) }}" method="POST">
                ...
                <div class="border border-indigo-100 p-4 rounded-md bg-indigo-50 mb-6 space-y-3">
                    ...
                    <div>
                        ...
                        <select id="deptSelect-{{ $request->id }}" class="w-full border-gray-300 rounded-md shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500"
                                onchange="filterUsersByDept('{{ $request->id }}')">
                    ...
                    <div>
                        ...
                        <select id="newUserSelect-{{ $request->id }}" class="w-full border-gray-300 rounded-md shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 disabled:bg-gray-100 disabled:text-gray-400" 
                                onchange="addNewUserField(this, 'new-users-container-{{ $request->id }}')" disabled>
                ...
                <div class="flex justify-end space-x-3 border-t pt-4">
                    <button type="button" onclick="closeModal('editModal-{{ $request->id }}')" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors">Cancel</button>
                    <button type="submit" class="px-6 py-2 text-sm font-bold text-white bg-indigo-600 rounded-md hover:bg-indigo-700 shadow-md transition-colors">Approve</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal-content reject-modal-content" onclick="event.stopPropagation();">
        ...
        <form id="rejectForm" method="POST" action="">
            ...
            <div class="flex justify-end space-x-3 pt-2">
                <button type="button" onclick="closeRejectModal()" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors">Cancel</button>
                <button type="submit" class="px-4 py-2 text-sm font-bold text-white bg-red-600 rounded-md hover:bg-red-700 shadow-sm transition-colors">Confirm Reject</button>
            </div>
        </form>
    </div>
</div>
